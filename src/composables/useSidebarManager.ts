...
interface SidebarContext {
  ...
  loadTableData: (tableName: string, connectionId: string, database: string, filter?: any, paneId?: string) => Promise<void>
  openQueryTab: (connectionId: string, database: string | null, paneId?: string) => void
  syncStoreForFetch: (connectionId: string, database: string) => void
}

export function useSidebarManager(ctx: SidebarContext) {
  const store = useConnectionStore()
  const { panes, activePaneId, getPane, getPaneTab, switchToTab, closeTab, refreshActiveTab, loadTableData, openQueryTab, syncStoreForFetch } = ctx
  ...
  function toggleConnection(connectionId: string) {
    if (expandedConnections.value.has(connectionId)) expandedConnections.value.delete(connectionId)
  ...
  async function createDatabase(connectionId: string) {
    ...
    try {
      await invoke('create_database', { connectionId, name: newDbName.value.trim() })
  ...
  async function importSql(connectionId: string, database: string) {
    ...
    try {
      ...
      const result = await invoke<{ executed: number; errors: string[] }>('import_sql', { connectionId, database, path })
  ...
  async function startExport() {
    ...
    try {
      ...
      const rows = await invoke<number>('export_database', {
        connectionId, database, mode: currentExportMode.value, path, tables: selectedExportTables.value,
      })
      exportResult.value = {
        success: true,
  ...
  function confirmSidebarTableAction(type: 'truncate' | 'drop', connectionId: string, database: string, tableName: string) {
  ...
  async function executeTableAction(disableFk: boolean) {
    ...
    try {
      if (type === 'drop') {
        await invoke('drop_table', { connection_id: connectionId, database, table: tableName, disable_fk_checks: disableFk })
        for (const pane of panes.value) {
          const related = pane.tabs.filter(t =>
            t.type === 'table' && (t as TableTab).tableName === tableName &&
            (t as TableTab).database === database && t.connectionId === connectionId,
          )
          related.forEach(t => closeTab(t.id, pane.id))
        }
        await store.fetchTablesForConnection(connectionId, database)
      } else {
        await invoke('truncate_table', { connectionId, database, table: tableName, disableFkChecks: disableFk })
        for (const pane of panes.value) {
          ...
          if (tab) {
            tab.pendingTruncate = false; tab.pendingChanges = {}; tab.pendingDeletions = {}
  ...
  const sidebarContextMenu = ref({ show: false, x: 0, y: 0, connection: null as Connection | null })
  const sidebarTableContextMenu = ref({ show: false, x: 0, y: 0, connectionId: '', database: '', tableName: '' })

  function openSidebarContextMenu(e: MouseEvent, conn: Connection) {
    e.preventDefault()
    sidebarContextMenu.value = { show: true, x: e.clientX, y: e.clientY, connection: conn }
    const close = () => { sidebarContextMenu.value.show = false; window.removeEventListener('click', close) }
    window.addEventListener('click', close)
  }

  function openSidebarTableContextMenu(e: MouseEvent, connectionId: string, database: string, tableName: string) {
    e.preventDefault(); e.stopPropagation()
    sidebarTableContextMenu.value = { show: true, x: e.clientX, y: e.clientY, connectionId, database, tableName }
    const close = () => { sidebarTableContextMenu.value.show = false; window.removeEventListener('click', close) }
  ...
  function blankConn(): Connection {
    return { id: uuidv4(), name: '', environment: 'LOCAL', mysql: { host: '127.0.0.1', port: 3306, user: 'root', password: '', database: '' } }
  }

  const newConn = ref<Connection>(blankConn())

  function openNewConnDialog() { newConn.value = blankConn(); showNewConnDialog.value = true }
  function openEditConnDialog(conn: Connection) { newConn.value = JSON.parse(JSON.stringify(conn)); showNewConnDialog.value = true }

  function handleDuplicateConnection(conn: Connection) {
    const dup = JSON.parse(JSON.stringify(conn))
    dup.id = uuidv4(); dup.name = `${conn.name} (Copy)`
    newConn.value = dup; showNewConnDialog.value = true; sidebarContextMenu.value.show = false
  }
  ...
  async function saveNewConn(conn: Connection, andConnect: boolean) {
    ...
    try {
      ...
      if (andConnect) { await store.connect(conn); expandedConnections.value.add(conn.id) }
  ...
  function confirmDeleteConn(id: string) {
    connToDelete.value = id; showDeleteConnDialog.value = true; sidebarContextMenu.value.show = false
  }
  ...
  return {
    ...
    closedConnections, filteredTables,
    connectSaved, toggleConnection, toggleDatabase, disconnectConn, createDatabase,
    // Import/Export
    isImporting, importResult, importProgress,
    ...
    importSql, openExportSelector, startExport,
    // Table actions
