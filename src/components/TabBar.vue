...
<template>
  ...
  >
    ...
    <button
      ...
      :title="tab.type === 'query' ? `Query · ${connectionNames[tab.connectionId] ?? ''}` : `${tab.tableName} · ${connectionNames[tab.connectionId] ?? ''} · ${tab.database}`"
    >
      ...
      <div class="flex flex-col items-start min-w-0 flex-1">
        ...
        <span class="text-[9px] font-medium truncate leading-tight opacity-50">
          {{ connectionNames[tab.connectionId] ?? '' }}<template v-if="tab.database"> · {{ tab.database }}</template>
        </span>
      </div>
      <span
        ...
        :class="[
          ...
          tab.id === activeTabId ? 'opacity-40 hover:opacity-100' : 'opacity-0 group-hover/tab:opacity-40 group-hover/tab:hover:opacity-100',
        ]"
      >
        <XIcon class="size-3" />
      </span>
    ...
    <template v-if="hasActiveTableTab">
      <div class="flex items-center gap-1 px-2 border-r h-full">
        <button
          ...
          :class="showFilters ? 'bg-primary/10 text-primary border-primary/20' : 'text-muted-foreground border-transparent hover:border-border hover:bg-muted/30'"
