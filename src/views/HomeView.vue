...
function openSidebarTableContextMenu(e: MouseEvent, connectionId: string, database: string, tableName: string) {
  e.preventDefault()
  e.stopPropagation()
...
function getPaneGroupedTabs(pane: PaneState) {
  const groups: { connectionId: string, database: string | null, tabs: AnyTab[] }[] = []
  for (const tab of pane.tabs) {
    const lastGroup = groups[groups.length - 1]
    if (lastGroup && lastGroup.connectionId === tab.connectionId && lastGroup.database === tab.database) {
      lastGroup.tabs.push(tab)
    } else {
      groups.push({ connectionId: tab.connectionId, database: tab.database, tabs: [tab] })
    }
  }
  return groups
}
...
async function loadTableData(tableName: string, connectionId: string, database: string, initialFilter?: any, paneId?: string) {
  const pane = getPane(paneId)

  if (!initialFilter) {
    const existing = pane.tabs.find(t => t.type === 'table' && (t as TableTab).tableName === tableName && (t as TableTab).database === database && t.connectionId === connectionId)
    if (existing) {
...
async function refreshActiveTab(paneId?: string) {
  ...
  try {
    await Promise.all([
      store.fetchTableData(tab.tableName, pane.page, pane.pageSize, tab.filters ?? null),
...
async function changePage(pane: PaneState, delta: number) {
  ...
  tab.page = pane.page
  syncStoreForFetch(tab.connectionId, tab.database)
  await store.fetchTableData(tab.tableName, pane.page, pane.pageSize, tab.filters)
  saveToActiveTab(pane)
}
...
async function handleDeleteTable(pane: PaneState) {
  ...
  const conn = getPaneConnection(pane)
  if (!tab || !conn) return
  const confirmed = confirm(`Are you sure you want to DROP TABLE \`${tab.tableName}\`? This cannot be undone.`)
  if (!confirmed) return
  try {
...
function filteredTables(connectionId: string, db: string) {
  ...
  return tbls.filter((t: any) => t.name.toLowerCase().includes(search.value.toLowerCase()))
}
...
async function importSql(connectionId: string, database: string) {
  ...
  try {
    unlisten = await listen<{ current: number; total: number; status: string }>('import-progress', (event) => {
      importProgress.value = event.payload
    })

    const result = await invoke<{ executed: number; errors: string[] }>('import_sql', {
      connectionId,
      database,
      path,
    })
...
const exportContext = ref<{ connectionId: string; database: string } | null>(null)
...
function openExportSelector(connectionId: string, database: string) {
  ...
  selectedExportTables.value = (store.openConnections[connectionId]?.tables[database] ?? []).map((t: any) => t.name)
...
function toggleAllTables() {
  ...
  const all = (store.openConnections[exportContext.value.connectionId]?.tables[exportContext.value.database] ?? []).map((t: any) => t.name)
...
const exportContextTables = computed(() => {
  if (!exportContext.value) return []
  return store.openConnections[exportContext.value.connectionId]?.tables[exportContext.value.database] ?? []
})
...
async function startExport() {
  ...
  exportProgress.value = { current: 0, total: 0, status: 'Initializing export...' }

  let unlisten: UnlistenFn | null = null
  try {
    unlisten = await listen<{ current: number; total: number; status: string }>('export-progress', (event) => {
    ...
    exportResult.value = { success: true, message: `Database exported successfully. ${rows} rows from ${selectedExportTables.value.length} tables included.` }
  } catch (e: any) {
...
const tableActionData = ref<{ type: 'truncate' | 'drop', connectionId: string, database: string, tableName: string } | null>(null)
...
function confirmSidebarTruncateTable() {
  const { connectionId, database, tableName } = sidebarTableContextMenu.value
  if (!connectionId || !database || !tableName) return
  tableActionData.value = { type: 'truncate', connectionId, database, tableName }
  tableActionDisableFk.value = false
  showTableActionDialog.value = true
  sidebarTableContextMenu.value.show = false
}
...
async function executeTableAction() {
  ...
  try {
    if (type === 'drop') {
      ...
      for (const pane of panes.value) {
        const relatedTabs = pane.tabs.filter(t => t.type === 'table' && (t as TableTab).tableName === tableName && (t as TableTab).database === database && t.connectionId === connectionId)
      ...
      for (const pane of panes.value) {
        const tab = pane.tabs.find(t => t.type === 'table' && (t as TableTab).tableName === tableName && (t as TableTab).database === database && t.connectionId === connectionId) as TableTab | undefined
        if (tab) {
          tab.pendingTruncate = false
          tab.pendingChanges = {}
          tab.pendingDeletions = {}
...
<template>
  <div class="h-full flex overflow-hidden bg-background">
    ...
    <aside class="w-72 flex flex-col border-r bg-muted/10">
        ...
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            ...
            <div>
              ...
              <span class="text-[10px] text-muted-foreground">{{ Object.keys(store.openConnections).length }} connection{{ Object.keys(store.openConnections).length !== 1 ? 's' : '' }}</span>
            </div>
          </div>
        </div>
        <div v-if="Object.keys(store.openConnections).length > 0" class="relative">
          <SearchIcon class="absolute left-2.5 top-1/2 -translate-y-1/2 size-3.5 text-muted-foreground" />
          <Input v-model="search" placeholder="Filter tables..." class="h-8 pl-8 text-xs bg-muted/50 border-none rounded-lg" />
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-2 custom-scrollbar">
        ...
        <template v-if="closedConnections.length > 0">
          <div class="px-2 py-1.5 mb-0.5">
            <span class="text-[10px] font-bold text-muted-foreground/50 uppercase tracking-widest">Saved</span>
          </div>
          <div
          ...
          >
            <div class="flex-1 flex items-center gap-2 px-2 py-1.5 min-w-0">
              <div class="size-1.5 rounded-full bg-muted-foreground/20 shrink-0"></div>
              <DatabaseIcon class="size-3.5 shrink-0 text-muted-foreground/40" />
              <span class="text-xs truncate flex-1 text-muted-foreground/70">{{ conn.name }}</span>
              <Badge variant="outline" :class="[getEnvColor(conn.environment), 'text-[9px] uppercase py-0 px-1 h-3.5 shrink-0']">
                {{ conn.environment }}
              </Badge>
            </div>
          </div>
          <div v-if="Object.keys(store.openConnections).length > 0" class="border-t border-muted/30 my-2"></div>
        </template>

        <!-- Empty state -->
        <div v-if="Object.keys(store.openConnections).length === 0 && closedConnections.length === 0" class="flex flex-col items-center justify-center py-12 px-4 text-center">
          <PlugZapIcon class="size-8 text-muted-foreground/30 mb-3" />
          <p class="text-xs text-muted-foreground/60 font-medium">No connections saved</p>
        </div>

        <!-- Connection nodes -->
        <div v-for="(connState, connId) in store.openConnections" :key="connId" class="mb-1">
          <!-- Connection header -->
          <div class="flex items-center gap-1 group rounded-md hover:bg-muted/40 transition-colors select-none">
            <button
              class="flex-1 flex items-center gap-2 px-2 py-1.5 min-w-0"
              @click="toggleConnection(connId as string)"
              @contextmenu="openSidebarContextMenu($event, connState.connection)"
            >
              <ChevronDownIcon v-if="expandedConnections.has(connId as string)" class="size-3.5 text-muted-foreground shrink-0 transition-transform" />
              <ChevronRightIcon v-else class="size-3.5 text-muted-foreground shrink-0 transition-transform" />
              <DatabaseIcon class="size-3.5 shrink-0 text-primary/70" />
              <span class="text-xs font-bold truncate flex-1 text-left">{{ connState.connection.name }}</span>
              <Badge variant="outline" :class="[getEnvColor(connState.connection.environment), 'text-[9px] uppercase py-0 px-1 h-3.5 shrink-0']">
          ...
          <!-- Databases -->
          <div v-if="expandedConnections.has(connId as string)" class="ml-4 mt-0.5 space-y-0.5">
            <!-- New DB form -->
            <div v-if="showNewDb === connId" class="px-1 pb-1">
              <div class="flex items-center gap-1">
                <Input
                  ...
                  @keyup.escape="showNewDb = null; newDbName = ''"
                />
                <button
                ...
                <button
                  ...
                  @click="showNewDb = null; newDbName = ''"
                >
            ...
            <button
              ...
              @click="showNewDb = connId as string; newDbName = ''"
            >
            ...
            <div v-for="db in connState.databases" :key="db">
              <div class="flex items-center gap-1 group/db rounded-md hover:bg-muted/30 transition-colors">
                <button
                  class="flex-1 flex items-center gap-2 px-2 py-1.5 min-w-0"
                  @click="toggleDatabase(connId as string, db)"
                >
                  <ChevronDownIcon v-if="expandedDatabases.has(dbKey(connId as string, db))" class="size-3 text-muted-foreground shrink-0" />
                  <ChevronRightIcon v-else class="size-3 text-muted-foreground/40 shrink-0" />
                  <DatabaseIcon class="size-3 shrink-0 text-muted-foreground/60" />
                  <span class="text-xs truncate flex-1 text-left font-medium">{{ db }}</span>
                </button>
                <!-- Import / Export / Query icons -->
                <div class="flex items-center gap-0.5 opacity-0 group-hover/db:opacity-100 transition-opacity shrink-0 mr-1">
                  <button
              ...
              <!-- Tables -->
              <div v-if="expandedDatabases.has(dbKey(connId as string, db))" class="ml-4 space-y-0.5 mt-0.5">
                <button
                  v-for="table in filteredTables(connId as string, db)"
                  :key="table.name"
                  @click="loadTableData(table.name, connId as string, db)"
                  @contextmenu="openSidebarTableContextMenu($event, connId as string, db, table.name)"
                  :class="[
                ...
                >
                  ...
                  <span
                    v-if="isTableOpenInAnyPane(table.name, db, connId as string)"
                    class="size-1.5 rounded-full shrink-0"
                    :class="isTableActiveInAnyPane(table.name, db, connId as string) ? 'bg-primary-foreground/60' : 'bg-primary/40'"
                  />
                </button>

                <div v-if="filteredTables(connId as string, db).length === 0 && search" class="px-2 py-1 text-[10px] text-muted-foreground/40 italic">
                  No matches
                </div>
              </div>
            </div>
      ...
      <!-- Sidebar footer -->
      <div class="p-3 border-t bg-background/30">
        <button
          @click="openNewConnDialog"
          class="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-primary/5 hover:bg-primary/10 border border-primary/10 hover:border-primary/20 text-primary text-xs font-bold transition-all"
        >
    ...
    <!-- Panes container -->
    <div ref="panesContainer" class="flex-1 flex min-w-0 overflow-hidden" :class="{ 'select-none': draggingPaneIdx !== null }">
      <template v-for="(pane, paneIdx) in panes" :key="pane.id">
        ...
        <!-- Pane -->
        <div
          class="flex flex-col min-w-0 overflow-hidden bg-background"
        ...
        >
          <!-- Tab bar -->
          <div v-if="pane.tabs.length > 0 || Object.keys(store.openConnections).length > 0" class="flex items-end border-b bg-muted/5 overflow-x-auto shrink-0 h-10">
            <button
              ...
              :title="tab.type === 'query' ? `Query · ${store.openConnections[tab.connectionId]?.connection.name}` : `${(tab as any).tableName} · ${store.openConnections[tab.connectionId]?.connection.name} · ${(tab as any).database}`"
            >
              <TerminalIcon v-if="tab.type === 'query'" class="size-3 shrink-0 opacity-60 mt-0.5" />
              <TableIcon v-else class="size-3 shrink-0 opacity-60 mt-0.5" />
              <div class="flex flex-col items-start min-w-0 flex-1">
                <span class="text-[11px] font-semibold truncate leading-tight">{{ tab.type === 'query' ? 'Query' : (tab as any).tableName }}</span>
                <span class="text-[9px] font-medium truncate leading-tight opacity-50">
                  {{ store.openConnections[tab.connectionId]?.connection.name }}<template v-if="tab.database"> · {{ tab.database }}</template>
                </span>
              </div>
              <span
                ...
                :class="[
                  ...
                  tab.id === pane.activeTabId ? 'opacity-40 hover:opacity-100' : 'opacity-0 group-hover/tab:opacity-40 group-hover/tab:hover:opacity-100'
                ]"
              >
                <XIcon class="size-3" />
              </span>
            ...
            <button
              ...
              @click="openQueryTab(Object.keys(store.openConnections)[0], null, pane.id)"
          ...
          <template v-if="getPaneTab(pane) && !isPaneActiveTabQuery(pane)">
            <!-- Table Header -->
            <header class="h-14 border-b flex items-center justify-between px-6 bg-background/50 backdrop-blur-sm shrink-0 relative z-30">
              <div class="flex items-center gap-3 min-w-0">
                <div class="size-9 rounded-lg bg-primary flex items-center justify-center text-primary-foreground shadow-inner">
                  <TableIcon class="size-5" />
                </div>
                <div class="min-w-0">
                  <div class="flex items-center gap-2">
                    <h1 class="text-lg font-bold truncate tracking-tight">{{ getPaneTab(pane)?.tableName }}</h1>
                    <Badge variant="secondary" class="text-[10px] font-bold uppercase tracking-wider py-0 px-1.5 h-4">{{ getPaneTab(pane)?.queryResult?.total_count }} rows</Badge>
                  </div>
                  <p class="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">
                    {{ getPaneConnection(pane)?.name }} · {{ getPaneTab(pane)?.database }}
                  </p>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <!-- View toggle -->
                <div class="flex items-center rounded-lg border bg-muted/30 p-0.5 gap-0.5">
                  <button
                    @click="pane.viewMode = 'content'"
                    :class="[
                      'flex items-center gap-1.5 px-3 h-7 rounded-md text-[11px] font-bold uppercase tracking-wider transition-all',
                      pane.viewMode === 'content' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
                    ]"
                  >
                    <LayoutListIcon class="size-3.5" /> Content
                  </button>
                  <button
                    @click="pane.viewMode = 'structure'"
                    :class="[
                      'flex items-center gap-1.5 px-3 h-7 rounded-md text-[11px] font-bold uppercase tracking-wider transition-all',
                      pane.viewMode === 'structure' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
                    ]"
                  >
                    <TablePropertiesIcon class="size-3.5" /> Structure
                  </button>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                  class="h-8 w-8 p-0"
                  :class="pane.showFilters ? 'bg-primary/10 text-primary border-primary/20' : 'text-muted-foreground'"
                  title="Toggle Filters"
                  @click="pane.showFilters = !pane.showFilters"
                >
                  <FilterIcon class="size-3.5" />
                </Button>
                <Button variant="outline" size="sm" class="h-8 w-8 p-0" title="Refresh" @click="refreshActiveTab(pane.id)">
            ...
            <FilterBar
              ...
              @apply="async (filters) => {
                ...
                if (!t) return
                syncStoreForFetch(t.connectionId, t.database)
                await store.fetchTableData(t.tableName, pane.page, pane.pageSize, filters)
                t.filters = filters
              ...
              @clear="async () => {
                ...
                if (!t) return
                syncStoreForFetch(t.connectionId, t.database)
                await store.fetchTableData(t.tableName, pane.page, pane.pageSize, null)
                t.filters = null
                t.queryResult = store.queryResult
              }"
            />
            ...
            <!-- Structure View -->
            <div v-if="pane.viewMode === 'structure'" class="flex-1 overflow-auto custom-scrollbar bg-muted/5">
                ...
                <tbody>
                  ...
                  >
                    <td class="px-4 py-3 border-b border-r font-mono text-sm font-semibold text-foreground whitespace-nowrap">
                      <div class="flex items-center gap-2">
                        <span v-if="(col as any).key === 'PRI'" class="text-[9px] font-black uppercase text-amber-400 bg-amber-400/10 px-1.5 py-0.5 rounded">PK</span>
                        <span v-else-if="(col as any).key === 'UNI'" class="text-[9px] font-black uppercase text-blue-400 bg-blue-400/10 px-1.5 py-0.5 rounded">UNI</span>
                        <span v-else-if="(col as any).key === 'MUL'" class="text-[9px] font-black uppercase text-purple-400 bg-purple-400/10 px-1.5 py-0.5 rounded">IDX</span>
                        {{ (col as any).field }}
                      </div>
                    </td>
                    <td class="px-4 py-3 border-b border-r font-mono text-xs text-primary whitespace-nowrap">{{ (col as any).field_type }}</td>
                    <td class="px-4 py-3 border-b border-r text-sm">
                      <span v-if="(col as any).nullable" class="text-[10px] font-bold text-green-400 uppercase">YES</span>
                      <span v-else class="text-[10px] font-bold text-muted-foreground/50 uppercase">NO</span>
                    </td>
                    <td class="px-4 py-3 border-b border-r text-xs text-muted-foreground whitespace-nowrap">{{ (col as any).key || '—' }}</td>
                    <td class="px-4 py-3 border-b border-r text-xs font-mono text-muted-foreground whitespace-nowrap">
                      <span v-if="(col as any).default_value === null" class="italic opacity-40">NULL</span>
                      <span v-else>{{ (col as any).default_value }}</span>
                    </td>
                    <td class="px-4 py-3 border-b border-r text-xs text-muted-foreground whitespace-nowrap">{{ (col as any).extra || '—' }}</td>
                    <td class="px-4 py-3 border-b text-xs whitespace-nowrap">
                      <span v-if="getFkMap(pane)[(col as any).field]" class="flex items-center gap-1 text-primary/70 font-mono">
                        <ArrowRightIcon class="size-3 shrink-0" />
                        {{ getFkMap(pane)[(col as any).field].table }}.{{ getFkMap(pane)[(col as any).field].column }}
                      </span>
            ...
            <!-- Data Table -->
            <div v-if="pane.viewMode === 'content'" class="flex-1 overflow-auto relative custom-scrollbar bg-muted/5">
              <div v-if="getPaneTab(pane)?.queryResult && getPaneTab(pane)?.queryResult?.rows?.length === 0" class="absolute inset-0 flex flex-col items-center justify-center p-12 text-center">
                <DatabaseIcon class="size-10 text-muted-foreground/20 mb-4" />
                <p class="text-lg font-bold text-foreground">No records</p>
                <p class="text-sm text-muted-foreground/60 max-w-[250px] mt-2">
              ...
              <template v-else>
                ...
                <thead>
                  <tr>
                    <th class="sticky top-0 left-0 z-30 bg-background/95 backdrop-blur-md px-2 py-3 border-b border-r text-center w-10">
                      <Trash2Icon class="size-3.5 mx-auto text-muted-foreground/40" />
                    </th>
                    <th
                      v-for="col in getPaneTab(pane)?.queryResult?.columns"
                      :key="(col as any).name"
                      class="sticky top-0 z-20 bg-background/95 backdrop-blur-md px-4 py-3 border-b border-r last:border-r-0 text-left min-w-[180px] whitespace-nowrap"
                    >
                      <div class="flex items-center gap-1.5">
                        <span class="block text-[10px] font-black tracking-widest text-foreground uppercase">{{ (col as any).name }}</span>
                        <span v-if="getPrimaryKey(pane) === (col as any).name" class="text-[8px] font-black text-amber-500 border border-amber-500/30 px-1 rounded">PK</span>
                      </div>
                      <span class="block text-[9px] font-bold uppercase tracking-tighter text-muted-foreground opacity-70">{{ (col as any).type_name }}</span>
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    v-for="(row, idx) in getPaneTab(pane)?.queryResult?.rows"
                    :key="idx"
                    class="hover:bg-primary/5 transition-colors group/row"
                    :class="[
                      (idx as number) % 2 === 0 ? 'bg-background/30' : 'bg-transparent',
                      getPaneTab(pane)?.pendingTruncate ? 'bg-destructive/20 opacity-70 grayscale' : '',
                      getPaneTab(pane)?.pendingDeletions[String((row as any)[getPrimaryKey(pane) || ''])] ? 'bg-destructive/20 text-destructive line-through' : ''
                    ]"
                  >
                    <td class="sticky left-0 z-10 bg-background/80 backdrop-blur-sm px-2 py-3 border-b border-r text-center">
                      <button
                        @click="toggleDeletion(pane, row)"
                        class="p-1 rounded hover:bg-destructive/20 text-muted-foreground/40 hover:text-destructive transition-colors"
                        :class="{ 'text-destructive opacity-100 bg-destructive/10': getPaneTab(pane)?.pendingDeletions[String((row as any)[getPrimaryKey(pane) || ''])] }"
                      >
                    ...
                    <td
                      v-for="col in getPaneTab(pane)?.queryResult?.columns"
                      :key="(col as any).name"
                      class="px-4 py-3 text-sm font-medium border-b border-r last:border-r-0 max-w-[300px] relative group/cell"
                      :class="[
                        getPaneTab(pane)?.pendingChanges[String((row as any)[getPrimaryKey(pane) || ''])]?.[(col as any).name] !== undefined ? 'bg-amber-500/10 border-amber-500/30' : '',
                        getPaneTab(pane)?.pendingDeletions[String((row as any)[getPrimaryKey(pane) || ''])] ? 'border-destructive/20' : ''
                      ]"
                    >
                      <span
                        v-if="(row as any)[(col as any).name] === null && getPaneTab(pane)?.pendingChanges[String((row as any)[getPrimaryKey(pane) || ''])]?.[(col as any).name] === undefined"
                        class="text-[10px] italic font-normal uppercase tracking-widest"
                        :class="getPaneTab(pane)?.pendingDeletions[String((row as any)[getPrimaryKey(pane) || ''])] ? 'text-destructive/50' : 'text-muted-foreground/30'"
                      >NULL</span>
                      <div v-else class="flex items-center gap-1.5 min-w-0 h-full">
                        <input
                          :value="getPaneTab(pane)?.pendingChanges[String((row as any)[getPrimaryKey(pane) || ''])]?.[(col as any).name] ?? (row as any)[(col as any).name]"
                          @input="(e) => updatePendingChange(pane, row, (col as any).name, (e.target as HTMLInputElement).value)"
                          class="bg-transparent border-none focus:ring-0 w-full p-0 truncate disabled:cursor-not-allowed"
                          :class="getPaneTab(pane)?.pendingDeletions[String((row as any)[getPrimaryKey(pane) || ''])] ? 'text-destructive font-bold' : 'text-foreground/80'"
                          :disabled="!getPrimaryKey(pane) || !!getPaneTab(pane)?.pendingDeletions[String((row as any)[getPrimaryKey(pane) || ''])]"
                        />
                        <button
                          v-if="getFkMap(pane)[(col as any).name]"
                          @click.stop="navigateToRelated(pane, getFkMap(pane)[(col as any).name].table, getFkMap(pane)[(col as any).name].column, (row as any)[(col as any).name])"
                          class="shrink-0 text-primary/40 hover:text-primary transition-colors opacity-0 group-hover/cell:opacity-100"
                      ...
                      <!-- Edit indicator -->
                      <div
                        v-if="getPaneTab(pane)?.pendingChanges[String((row as any)[getPrimaryKey(pane) || ''])]?.[(col as any).name] !== undefined"
              ...
              <div v-if="!getPrimaryKey(pane)" class="p-8 text-center bg-muted/20 border-t border-dashed mt-auto">
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <span class="text-xs font-bold uppercase tracking-widest text-foreground">
                  ...
                  <template v-else>
                    {{ Object.keys(getPaneTab(pane)?.pendingChanges || {}).length }} Updates &amp;
                    {{ Object.keys(getPaneTab(pane)?.pendingDeletions || {}).length }} Deletions Pending
                  </template>
                </span>
              </div>
              ...
              <div class="flex items-center gap-2">
                <div class="flex items-center gap-2 mr-2">
                  <label class="flex items-center gap-2 cursor-pointer group">
                    <input type="checkbox" v-model="disableFkChecks" class="size-3.5 rounded border-input accent-primary" />
                    <span class="text-[10px] font-bold text-muted-foreground group-hover:text-foreground transition-colors uppercase tracking-tight">Disable FK Checks</span>
                  </label>
                </div>
                <Button variant="ghost" size="sm" class="h-8 text-xs font-bold uppercase tracking-tight" @click="discardChanges(pane)">Discard</Button>
                <Button size="sm" class="h-8 px-4 text-xs font-bold uppercase tracking-tight shadow-lg" :disabled="isSaving" @click="applyChanges(pane)">
            ...
            <!-- Pagination Footer -->
            <footer v-if="pane.viewMode === 'content'" class="h-12 border-t flex items-center justify-between px-6 bg-background shrink-0">
              <div class="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
                Showing {{ pane.page * pane.pageSize + 1 }} - {{ Math.min((pane.page + 1) * pane.pageSize, getPaneTab(pane)?.queryResult?.total_count || 0) }} of {{ getPaneTab(pane)?.queryResult?.total_count }}
              </div>
              <div class="flex items-center gap-2">
                <Button
                  variant="outline"
                ...
                >PREVIOUS</Button>
                <div class="text-[10px] font-bold px-3">PAGE {{ pane.page + 1 }}</div>
                <Button
                  variant="outline"
                  size="sm"
                  class="h-7 text-[10px] font-bold px-4"
          ...
          <!-- Empty State -->
          <div v-else-if="!isPaneActiveTabQuery(pane)" class="flex-1 flex flex-col items-center justify-center p-12 text-center bg-muted/5">
            <div class="size-24 rounded-full bg-muted/20 flex items-center justify-center mb-8 border border-dashed border-muted-foreground/20">
            ...
            <div class="flex gap-4">
              <div class="flex flex-col items-center gap-2 px-6 py-4 rounded-2xl bg-background border shadow-sm">
                <span class="text-xs font-bold uppercase tracking-widest text-muted-foreground">Connections</span>
                <span class="text-xl font-black text-primary">{{ Object.keys(store.openConnections).length }}</span>
              </div>
              <div class="flex flex-col items-center gap-2 px-6 py-4 rounded-2xl bg-background border shadow-sm">
                <span class="text-xs font-bold uppercase tracking-widest text-muted-foreground">Open Tabs</span>
                <span class="text-xl font-black text-primary">{{ pane.tabs.length }}</span>
              </div>
            </div>
          </div>
    ...
    <Dialog :open="showTableSelector" @update:open="(val: boolean) => !val && (showTableSelector = false)">
      <DialogContent class="sm:max-w-md max-h-[85vh] flex flex-col p-0 overflow-hidden shadow-2xl border-primary/10">
        <DialogHeader class="p-6 pb-4 bg-background border-b relative z-20">
          ...
          <DialogDescription>
            Configure your export for <span class="font-bold text-foreground">`{{ exportContext?.database }}`</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar bg-background">
          ...
          <div class="px-6 py-4">
            <div class="flex items-center justify-between mb-3 sticky top-0 bg-background py-1 z-10 border-b">
              ...
              <div class="flex items-center gap-3">
                <span class="text-[10px] font-bold text-muted-foreground">{{ selectedExportTables.length }} selected</span>
                <button @click="toggleAllTables" class="text-[10px] font-black text-primary uppercase hover:underline">
                  {{ selectedExportTables.length === exportContextTables.length ? 'None' : 'All' }}
                </button>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-1">
              ...
              >
                ...
                <span class="text-sm font-medium truncate">{{ (table as any).name }}</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 py-4 border-t bg-muted/10 flex flex-row items-center justify-between gap-4">
          <Button variant="ghost" class="text-xs font-bold uppercase tracking-wider h-9" @click="showTableSelector = false">Cancel</Button>
          <Button
    ...
    <Dialog :open="isImporting">
      <DialogContent class="sm:max-w-md" :hide-close="true">
        <DialogHeader>
          ...
          <DialogDescription>Please wait while the SQL file is being imported.</DialogDescription>
        </DialogHeader>
        <div class="py-6">
          <div class="flex items-center justify-between mb-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">
            <span>{{ importProgress.status }}</span>
            <span v-if="importProgress.total">{{ Math.round((importProgress.current / importProgress.total) * 100) }}%</span>
          </div>
    ...
    <Dialog :open="!!importResult" @update:open="(val: boolean) => !val && (importResult = null)">
      <DialogContent class="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle :class="importResult?.errors.length ? 'text-destructive' : 'text-green-500'">
            {{ importResult?.errors.length ? 'Import finished with errors' : 'Import successful' }}
          </DialogTitle>
          <DialogDescription>
            {{ importResult?.executed }} statements executed successfully.
          </DialogDescription>
        </DialogHeader>
        <div v-if="importResult?.errors.length" class="mt-4 max-h-[300px] overflow-y-auto custom-scrollbar rounded-md border bg-muted/30 p-4">
          <div class="text-[10px] font-black uppercase tracking-widest text-destructive mb-2">Error Log:</div>
          <div v-for="(err, i) in importResult.errors" :key="i" class="text-xs font-mono mb-2 last:mb-0 break-all border-b border-muted last:border-0 pb-2">
            {{ err }}
          </div>
        </div>
    ...
    <Dialog :open="isExportingDb">
      <DialogContent class="sm:max-w-md" :hide-close="true">
        <DialogHeader>
          ...
          <DialogDescription>Please wait while the data is being exported.</DialogDescription>
        </DialogHeader>
        <div class="py-6">
          <div class="flex items-center justify-between mb-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">
            <span>{{ exportProgress.status }}</span>
            <span v-if="exportProgress.total">{{ Math.round((exportProgress.current / exportProgress.total) * 100) }}%</span>
          </div>
          <div class="h-2 w-full bg-muted rounded-full overflow-hidden">
            <div
              class="h-full bg-primary transition-all duration-300 ease-out"
    ...
    <Dialog :open="!!exportResult" @update:open="(val: boolean) => !val && (exportResult = null)">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          <DialogTitle :class="exportResult?.success ? 'text-green-500' : 'text-destructive'">
    ...
    <!-- New Connection Dialog -->
    <Dialog :open="showNewConnDialog" @update:open="(val: boolean) => !val && (showNewConnDialog = false)">
      <DialogContent class="sm:max-w-lg overflow-y-auto max-h-[90vh]">
            ...
            <div v-if="sshEnabled" class="space-y-3 pl-1">
              ...
              <div v-if="sshAuthType === 'key'" class="space-y-3">
                <div class="space-y-2">
                  ...
                  <Input v-model="sshForm.private_key_path" placeholder="~/.ssh/id_rsa" />
                </div>
                <div class="space-y-2">
        ...
        <div v-if="testConnResult" :class="[
          'text-xs px-3 py-2 rounded-md font-medium',
          testConnResult.ok ? 'bg-green-500/10 text-green-500' : 'bg-destructive/10 text-destructive'
        ]">
          {{ testConnResult.msg }}
        </div>

        <div class="flex items-center justify-between pt-4 border-t">
          <Button variant="ghost" @click="showNewConnDialog = false">Cancel</Button>
          <div class="flex gap-2">
            <Button variant="outline" :disabled="isTestingConn || isSavingConn" @click="testNewConn">
              {{ isTestingConn ? 'Testing...' : 'Test' }}
            </Button>
            <Button variant="outline" :disabled="isSavingConn" @click="saveNewConn(false)">Save only</Button>
            <Button :disabled="isSavingConn || !newConn.name" @click="saveNewConn(true)">Save &amp; Connect</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
    ...
    <Dialog :open="showDeleteConnDialog" @update:open="(val: boolean) => !val && (showDeleteConnDialog = false)">
      <DialogContent class="sm:max-w-[400px]">
        ...
        <DialogFooter class="gap-2 sm:gap-0">
          <Button variant="ghost" @click="showDeleteConnDialog = false">Cancel</Button>
    ...
    <Dialog :open="showTableActionDialog" @update:open="(val: boolean) => !val && !isExecutingTableAction && (showTableActionDialog = false)">
      <DialogContent class="sm:max-w-[420px]">
        <DialogHeader>
          <DialogTitle class="flex items-center gap-2">
            <Trash2Icon v-if="tableActionData?.type === 'truncate'" class="size-5 text-destructive" />
            <XIcon v-else class="size-5 text-destructive" />
            {{ tableActionData?.type === 'truncate' ? 'Truncate' : 'Drop' }} Table
          </DialogTitle>
          <DialogDescription class="pt-2">
            Are you sure you want to <strong class="text-foreground">{{ tableActionData?.type === 'truncate' ? 'TRUNCATE' : 'DROP' }}</strong> table <code class="bg-muted px-1.5 py-0.5 rounded font-bold text-foreground">{{ tableActionData?.tableName }}</code>?<br/>
            <span v-if="tableActionData?.type === 'truncate'" class="text-muted-foreground mt-2 block">All data will be lost. This cannot be undone.</span>
            <span v-else class="text-muted-foreground mt-2 block">This table and all its data will be permanently deleted.</span>
          </DialogDescription>
        </DialogHeader>

        <div class="py-4">
          <label class="flex items-start gap-3 cursor-pointer group bg-muted/20 p-3 rounded-lg border border-border hover:bg-muted/40 transition-colors">
            <div class="mt-0.5 shrink-0">
              <input type="checkbox" v-model="tableActionDisableFk" class="size-4 rounded border-input accent-destructive cursor-pointer" />
            </div>
            <div class="flex flex-col min-w-0">
              <span class="text-sm font-bold text-foreground transition-colors">Disable Foreign Key Checks</span>
              <span class="text-xs text-muted-foreground leading-relaxed mt-0.5">Allows truncating/dropping tables referenced by other tables. Use with caution.</span>
            </div>
          </label>
        </div>

        <DialogFooter class="gap-2 sm:gap-0 mt-2">
          <Button variant="ghost" @click="showTableActionDialog = false" :disabled="isExecutingTableAction">Cancel</Button>
          <Button variant="destructive" @click="executeTableAction" :disabled="isExecutingTableAction" class="min-w-[100px] font-bold">
    ...
    <div
      ...
      class="fixed z-[100] min-w-[160px] bg-background/95 backdrop-blur-md border rounded-lg shadow-xl p-1 animate-in fade-in zoom-in-95 duration-100"
      :style="{ left: sidebarTableContextMenu.x + 'px', top: sidebarTableContextMenu.y + 'px' }"
    >
      <div class="px-2 py-1 mb-1 border-b">
        <span class="text-[10px] font-black uppercase tracking-widest text-muted-foreground truncate block">
